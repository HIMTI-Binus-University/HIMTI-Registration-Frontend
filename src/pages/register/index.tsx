...
export default function RegisterPage() {
  ...
  const verificationPanel = () => <div className="rounded-2xl border border-brand-blue/15 bg-brand-pale/40 p-4 md:col-span-2" aria-live="polite">
    ...
    {!emailVerified && <>{!verificationSent ? <Button type="button" variant="outline" className="mt-4 min-h-11 w-full bg-white sm:w-auto" disabled={!data.binusEmail} onClick={() => setVerificationSent(true)}>Send verification link</Button> : <div className="mt-4"><p className="break-words text-xs leading-5 text-brand-slate">A verification link was sent to <strong>{data.binusEmail}</strong>. Open the email and click the link, then return here.</p><p className="mt-2 text-xs text-brand-slate">Verification is simulated while the verification service is being connected.</p><Button type="button" className="mt-4 min-h-11 w-full sm:w-auto" onClick={() => setEmailVerified(true)}>Check verification status</Button><button type="button" className="mt-3 block min-h-11 w-full rounded-lg text-sm font-semibold text-brand-blue focus:outline-none focus:ring-2 focus:ring-ring sm:w-auto sm:px-2" onClick={() => setVerificationSent(true)}>Resend verification link</button></div>}</>}
  </div>;

  const institutionFields = () => {
    if (data.userType === "Student" && data.institutionType === "BINUS") return <div className="grid gap-4 md:grid-cols-2"><Field label="NIM" name="nim" value={data.nim} onChange={update} /><Field label="BINUSian batch" name="batch" value={data.batch} onChange={update} placeholder="e.g. 28" /><Field label="BINUS email" name="binusEmail" value={data.binusEmail} onChange={update} type="email" placeholder="name@binus.ac.id" />{verificationPanel()}<SelectField label="BINUS region" name="region" value={data.region} options={regions} onChange={update} /><SelectField label="BINUS major" name="major" value={data.major} options={binusMajors} onChange={update} /></div>;
    if (data.userType === "Student") return <div className="grid gap-4 md:grid-cols-2"><Field label="University" name="university" value={data.university} onChange={update} /><Field label="Student ID / NIM" name="studentId" value={data.studentId} onChange={update} /><Field label="Major" name="major" value={data.major} onChange={update} /></div>;
    if (data.institutionType === "BINUS") return <div className="grid gap-4 md:grid-cols-2"><Field label="BINUS email" name="binusEmail" value={data.binusEmail} onChange={update} type="email" placeholder="name@binus.ac.id" />{verificationPanel()}<SelectField label="BINUS region" name="region" value={data.region} options={regions} onChange={update} /><Field label={data.userType === "Lecturer" ? "Department / program" : "Affiliation / role"} name={data.userType === "Lecturer" ? "department" : "affiliation"} value={data.userType === "Lecturer" ? data.department : data.affiliation} onChange={update} /></div>;
  ...
  return <main className="min-h-screen bg-background px-3 py-5 sm:px-6 sm:py-8"><div className="mx-auto max-w-2xl"><div className="mb-5 flex items-center justify-between gap-3"><Link to="/" className="flex min-w-0 items-center gap-2 rounded-lg text-sm font-bold text-brand-navy focus:outline-none focus:ring-2 focus:ring-ring"><span className="grid size-8 shrink-0 place-items-center overflow-hidden rounded-lg bg-brand-navy p-1"><img src="/himti-icon.svg" alt="" /></span><span className="truncate">HIMTI registration</span></Link><Link to="/" className="shrink-0 rounded-lg px-2 py-2 text-xs font-semibold text-brand-blue focus:outline-none focus:ring-2 focus:ring-ring sm:text-sm">Back home</Link></div><section className="rounded-2xl border border-white/80 bg-white/95 p-4 shadow-[0_24px_70px_-35px_rgba(0,33,79,0.45)] sm:rounded-3xl sm:p-8"><div><p className="section-label">Join the community</p><h1 className="mt-2 text-2xl font-bold tracking-[-0.04em] text-brand-navy sm:text-3xl">Tell us about yourself</h1><p className="mt-4 text-sm font-bold text-brand-blue sm:hidden">Step {step + 1} of 4 <span className="text-brand-slate">· {steps[step]}</span></p></div><div className="mt-5" aria-label={`Registration progress, step ${step + 1} of 4: ${steps[step]}`}><div className="flex gap-1.5">{steps.map((label, index) => <div key={label} className="flex-1" aria-current={index === step ? "step" : undefined}><div className={`h-1.5 rounded-full transition-colors ${index <= step ? "bg-brand-blue" : "bg-brand-blue/10"}`} /><span className={`mt-2 hidden text-[11px] font-semibold sm:block ${index === step ? "text-brand-blue" : "text-brand-slate"}`}>{label}</span></div>)}</div><p className="mt-3 hidden text-right text-xs font-bold text-brand-blue sm:block">Step {step + 1} of 4</p></div>{pathNotice && <div role="status" className="mt-6 rounded-xl border border-brand-blue/15 bg-brand-pale px-4 py-3 text-sm font-medium text-brand-navy">{pathNotice}</div>}{errors.length > 0 && <div ref={firstError} tabIndex={-1} role="alert" className="mt-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-800">{errors[0]}. Check the highlighted fields before continuing.</div>}<div className="mt-7">
    ...
    {step === 1 && <div><h2 className="text-lg font-bold text-brand-navy">Personal information</h2><p className="mt-1 text-sm text-brand-slate">Use details you can access regularly.</p><div className="mt-5 grid gap-4 md:grid-cols-2"><Field label="Full name" name="name" value={data.name} onChange={update} /><Field label="Phone number" name="phone" value={data.phone} onChange={update} type="tel" /><Field label="Personal email" name="personalEmail" value={data.personalEmail} onChange={update} type="email" /><Field label="LINE ID" name="lineId" value={data.lineId} onChange={update} required={false} /></div></div>}
  ...
  </div><div className="mt-8 flex items-center gap-2 border-t border-brand-blue/10 pt-5"><Button type="button" variant="outline" className={`min-h-11 px-3 sm:px-5 ${step === 0 ? "invisible" : ""}`} onClick={() => { setErrors([]); setStep((current) => current - 1); }}><ArrowLeft className="mr-1 size-4 sm:mr-2" /> Back</Button>{step < 3 ? <Button type="button" className="min-h-11 flex-1 sm:ml-auto sm:flex-none" onClick={next}>Continue <ArrowRight className="ml-2 size-4" /></Button> : <Button type="button" className="min-h-11 flex-1 sm:ml-auto sm:flex-none" onClick={submit}>Submit registration <Send className="ml-2 size-4" /></Button>}</div></section></div></main>;
}
