...
export function useProfile() {
  return useQuery({
    queryKey: ["profile"],
    queryFn: () => apiClient.get<Profile>(apiPaths.profile).then((r) => r.data),
    retry: false,
  });
}
...
export function useRegistrationOptions() {
  return useQuery({
    ...
    queryFn: () =>
      apiClient
        .get<{ data: RegistrationOptions }>(apiPaths.options)
        .then((r) => r.data.data),
  });
}
...
export function useCompleteProfile() {
  ...
  return useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      apiClient.patch(apiPaths.completeProfile, payload),
...
export function useUpdateProfile() {
  ...
  return useMutation({
    mutationFn: (payload: {
      ...
      phoneNumber: string;
      lineId: string;
    }) => apiClient.patch(apiPaths.updateProfile, payload),
    onSuccess: () => client.invalidateQueries({ queryKey: ["profile"] }),
  });
}
...
export function useSendVerification() {
  return useMutation({
    mutationFn: (email: string) =>
      apiClient.post(apiPaths.sendVerification, { email }),
  });
}
