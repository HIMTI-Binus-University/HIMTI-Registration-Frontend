...
type TicketListResponse = operations["listMyEventTicketsV1"]["responses"][200]["content"]["application/json"];
type TicketResponse = operations["getMyEventTicketV1"]["responses"][200]["content"]["application/json"];
type CredentialResponse = operations["getMyEventTicketCredentialV1"]["responses"][200]["content"]["application/json"];
...
const ticketPath = (ticketId: string) =>
  `/api/v1/me/event-tickets/${encodeURIComponent(ticketId)}`;
...
export function useMyTickets() {
  return useQuery({
    ...
    queryFn: () =>
      apiClient
        .get<TicketListResponse>("/api/v1/me/event-tickets")
        .then(({ data }) => data.data),
